...
async def email_detect_handler(
    input_data: EmailDetectInput, request: Request
) -> dict[str, Any]:
    ...
    try:
        ...
        raise AbortException(status=400, message=str(e), message_detail=tb)

    except (TimeoutError, RuntimeError):
        tb = traceback.format_exc()
        raise AbortException(status=400, message="ERROR", message_detail=tb)
