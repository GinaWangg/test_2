...
class EmailDetectPipeline:
    ...
    async def _generate_check_info(
        ...
        # Check API success
        api_kb_log_check = [
            0 if value[1].get('success') else 1
            for value in split_task_result
        ]
    ...
    def _generate_gpt_extract(self, email_split_info: list[dict]) -> list[dict]:
        ...
        for lst in email_split_info:
            if lst['extract_type'] == 'summary':
                continue
            if (
                lst['intent'] == 'Technical Support' and
                lst['top1_kb_similarity'] >= 0.695
            ):
        ...
        note_case3_target = [
            lst for lst in email_split_info
            if lst['extract_type'] == 'summary'
        ][0]
        ...
        tmp_dict = {}
        if (
            note_case3_target_intent == 'Technical Support' and
            note_case3_target['top1_kb_similarity'] >= 0.695
        ):
            ...
            for lst in email_split_info:
                if lst['extract_type'] == 'summary':
                    continue
                if (
                    note_case3_target_intent == lst['intent'] and
                    note_case3_target_top1_kb == lst['top1_kb'] and
            ...
            note_case3_target_intent not in [
                'Technical Support', 'Only Chat', 'Greeting'
            ] and
            note_case3_target['intent_similarity'] >= 0.6
        ):
            note_case3_max_similarity = 0
    ...
    def _num_recode(
        self,
        email_split_sentence: list[dict],
        email_split_info: list[dict]
        ...
        for item in email_split_sentence:
            ...
            if key not in output_email_split_sentence:
                ...
                index_transform_dict[raw_index] = (
                    output_email_split_sentence[key]["extract_type_num"]
                )
        ...
        for data in email_split_info:
            ...
            data["extract_type_num"] = (
                index_transform_dict.get(num_org, None)
            )
